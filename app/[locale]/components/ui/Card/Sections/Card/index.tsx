...
export default function CardSection({ projects, theme, className }: CardSectionProps) {
			...
			{sections.map((section, index) => (
				<div key={index} className={`w-full ${heights[section.type]}`}>
					{section.type === "single" && (
						<div
							className={`flex ${
								section.projects.length > 1 ? "flex-row gap-8" : ""
							} w-full h-full`}
						>
							{section.projects.map((project, i) => (
								<div
									key={i}
									className={`${section.projects.length > 1 ? "w-1/2" : "w-full"} h-full`}
								>
					...
					{section.type === "full" && <CardWork {...section.projects[0]} theme={theme} />}
					{section.type === "double" && (
